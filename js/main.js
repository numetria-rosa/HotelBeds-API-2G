...
-------------------------------------------------------------------*/
  ...
  window.onload = function () {
    $(".childselect0").change(function (e) {
      $(".age-wrapper0").remove();

      for (let l = 1; l <= $(this).val(); l++) {
        $(this)
          .parent()
          .after(
            '<div class="mr-5 age-wrapper0 d-flex y-gap-10 justify-start items-center"><div><div class="text-15 fw-500">Age</div>  <select class="ageselect0-' +
              l +
              ' rounded-4 border-light px-5 h-50 text-14"><option value="0">0</option><option value="1">1</option> <option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option><option value="7">7</option><option value="8">8</option><option value="9">9</option><option value="10">10</option><option value="11">11</option><option value="12">12</option></select></div><div>'
          );
      }
    });

    $("#RoomsSelect").change(function (e) {
      if ($(this).val() == 1) {
        $(".js-count-room").html("1 room");
      } else {
        $(".js-count-room").html($(this).val() + " rooms");
      }

      $(".pax-wrapper").empty();

      for (let i = 1; i <= $(this).val() - 1; i++) {
        $(".pax-wrapper").append(
          '<div class="d-flex y-gap-10 justify-start items-center"><div><div style="width:47px"></div></div><div class="mr-5"><div class="text-15 fw-500">Adults</div><select class="adultsselect' +
            i +
            ' rounded-4  border-light px-5 h-50 text-14"><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option></select>                              </div>                             <div class="mr-5"><div class="text-15  fw-500">Children</div><select class="childselect' +
            i +
            ' rounded-4 border-light px-5 h-50 text-14"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option></select></div></div>'
        );
      }

      for (let y = 1; y <= $(this).val() - 1; y++) {
        $(".childselect" + y).change(function (e) {
          $(".age-wrapper" + y).remove();

          for (let x = 1; x <= $(this).val(); x++) {
            $(this)
              .parent()
              .after(
                '<div class="mr-5 age-wrapper' +
                  y +
                  ' d-flex y-gap-10 justify-start items-center"><div ><div class="text-15 fw-500">Age</div><select class="ageselect' +
                  y +
                  "-" +
                  x +
                  ' rounded-4 border-light px-5 h-50 text-14"><option value="0">0</option><option value="1">1</option> <option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option><option value="7">7</option><option value="8">8</option><option value="9">9</option><option value="10">10</option><option value="11">11</option><option value="12">12</option></select></div></div>'
    ...
    $(".mainSearch__submit").click(function (e) {
      if (window.search == true) {
        ...
        var source = ($(this).attr("ctry") !== undefined) ? ($(this).attr("ctry")) : ("search");
        var place = ($(this).attr("landmark") !== undefined) ? ($(this).attr("landmark")) : ("destination");
        ...
        let rooms = $("#RoomsSelect").val();
        let adults = $(".adultsselect" + 0).val();
        let children = $(".childselect" + 0).val();
        let age = $(".ageselect" + 0 + "-" + 1).val();
        ...
        for (let i = 0; i < $("#RoomsSelect").val(); i++) {
          ...
          if ($(".childselect" + i).val() > 0) {
            dataStringSearchPax =
              ...
              "&";
            urlParams =
              urlParams + "Children" + i + "=" + $(".childselect" + i).val();
            ...
            urlParams = urlParams + "Children" + i + "=0";
          }
        }
        ...
        $.ajax({
          ...
          success: function (json) {
            ...
            WORKPATH +
            "hotel-list.php?source="+source+"&Place="+place+"&CityName=" +
              window.searchvalue +
              "&City=" +
              selCityHB +
              "&CheckIn=" +
    ...
    $(".mainFilter__submit").click(function (e) {
      if (window.search == true) {
        $(this).prop("disabled", true);
        $(".error").html("");
        $(".mainFilter__submit > i").attr("class", "");
        $(".mainFilter__submit > i").addClass(
          "fa fa-spinner fa-spin text-20 mr-10"
        );
        ...
        var Filters = [];
        var arrBoards = [];
        var arrAccommodations = [];
        $("input:checkbox[name*=boards]:checked").each(function () {
          arrBoards.push($(this).val());
        });
        $("input:checkbox[name*=Accommodations]:checked").each(function () {
          arrAccommodations.push($(this).val());
        });

        Filters.push($(".StarRating .active-star").val());
        let lowprice = $(".js-lower").text();
        let upprice = $(".js-upper").text();
        Filters.push([lowprice.replace("€ ", ""), upprice.replace("€ ", "")]);
    ...
    $(".searchBydest").click(function () {
      window.isoCode = $(this).attr("attr");
      window.searchvalue = $(this).attr("name");
      window.zoneCode =  $(this).attr("code");
      ...
      Checkin +
      "&CheckOut=" +
      Checkout +"&Rooms=1&Adults0=1&Children0=0";
    });

    $(".searchByzone").click(function () {
      window.isoCode = $(this).attr("attr");
      window.searchvalue = $(this).attr("name");
      let Checkin = window.start;
      let Checkout = window.end;
      ...
      (window.location.href = WORKPATH + "hotel-list.php");
    });
  ...
  function liveSearch() {
    ...
    targets.forEach((el) => {
      ...
      const resultshotels = el.querySelector(".js-results-hotels");
      let searchTerm = "";
      ...
      $("#search").keyup(function (event) {
        ...
        if (searchTerm.length > 3) {
          typingTimer = setTimeout(function () {
            doneTyping(searchTerm, results, search, el, resultshotels);
          }, doneTypingInterval);
        }
      });
    });

    function doneTyping(searchTerm, results, search, el, resultshotels) {
      results.innerHTML = "";
      resultshotels.innerHTML = "";
      $(".lds-dual-ring").show();
      $(".ring").show();
      var dataStringHB = "query=" + searchTerm + "&action=searchcountryORcity";
      if (searchTerm.length > 3) {
        $(".searchMenu-loc__field").show();
        $.ajax({
          type: "POST",
          url: "actions.php",
          data: dataStringHB,
          ...
          success: function (json) {
            ...
            $(".ring").hide();
            $(".js-results").show();
            $(".js-results-hotels").show();
            var response = JSON.parse(json);
            data = [];
            data2 = [];
            data3 = [];
            data.push(...JSON.parse(response.cities));
            data2.push(...JSON.parse(response.zones));
            data3.push(...JSON.parse(response.hotels));

            showList(searchTerm, results, resultshotels);
            ...
            results.querySelectorAll(".js-search-option").forEach((option) => {
              ...
              const isoCode = option.querySelector(
                ".js-search-option-target-isocode"
              )?.innerHTML;

              const zoneCode = option.querySelector(
                ".js-search-option-target-zonecode"
              )?.innerHTML;

              option.addEventListener("click", () => {
                window.search = true;
                window.isoCode = isoCode;
                window.zoneCode = zoneCode;
                window.searchvalue = title;
                search.value = title;
                $(".error").html("");
                el.querySelector(".js-popup-window").classList.remove(
                  "-is-active"
                );
              });
            });
            ...
            resultshotels
              ...
              .forEach((option) => {
                ...
                const hotelCode = option.querySelector(
                  ".js-search-option-target-hotelcode"
                )?.innerHTML;

                option.addEventListener("click", () => {
                  window.search = true;
                  window.hotelCode = hotelCode;
                  window.hotel = title;
                  search.value = title;
                  $(".error").html("");
                  el.querySelector(".js-popup-window").classList.remove(
        ...
        $(".js-popup-window").addClass("-is-active");
      } else {
        $(".js-popup-window").removeClass("-is-active");
        $(".searchMenu-loc__field").hide();
      }
    }
    const showList = (searchTerm, resultsEl, resultshotels) => {
      resultsEl.innerHTML = "";
      resultshotels.innerHTML = "";
      resultsEl.innerHTML += "<h5>Destinations & zones</h5>";
      resultshotels.innerHTML += "<h5>Hotels</h5>";

      data.forEach((e) => {
        const div = document.createElement("div");

        div.innerHTML = `
          <button class="-link d-block col-12 text-left rounded-4  py-3-3 js-search-option">
            <div class="d-flex">
              ...
              <div class="ml-10">
                ...
                <div style="display:none" class="js-search-option-target-isocode">${e.data}</div>
              </div>
            </div>
          </button>
        `;
      ...
      data2.forEach((el) => {
        ...
        div.innerHTML = `
          <button class="-link d-block col-12 text-left rounded-4  py-3-3 js-search-option">
            <div class="d-flex">
              <div class="icon-destination text-light-1 text-20 pt-4"></div>
              <div class="ml-10">
                <div class="text-15 lh-12 fw-500 js-search-option-target">${el.name}</div>
                <div class="text-14 lh-12 text-light-1 mt-5">${el.destination}(${el.countrycode})</div>
                <div style="display:none" class="js-search-option-target-isocode">${el.destinationcode}</div>
                <div style="display:none" class="js-search-option-target-zonecode">${el.code}</div>
              </div>
            </div>
          </button>
        `;
        ...
        div.innerHTML = `
          <button class="-link d-block col-12 text-left rounded-4  py-3-3 js-search-option">
            <div class="d-flex">
              ...
              <div class="ml-10">
                <div class="text-15 lh-12 fw-500 js-search-option-target">${el.name}</div>
                <div class="text-14 lh-12 text-light-1 mt-5">${el.destinationCode}(${el.countryCode})</div>
                <div style="display:none" class="js-search-option-target-hotelcode">${el.code}</div>
              </div>
            </div>
          </button>
        `;
        resultshotels.appendChild(div);
      });
    };
  }
