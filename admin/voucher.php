...
if ($NB == 0) {
    header("location:" . $WORKPATH . "admin/");
} else {
    ...
    if ($BOOK_LFN != '') {
        $BOOK_CLIENTPRINCIPAL = '<tr>
            <td style="width:30%; text-align:right; padding-right:5px;">Client Principal</td>
            <td style="width:70%; text-align:left; font-weight:bold; padding-left:5px;">' . $BOOK_LT . ' ' . $BOOK_LFN . ' ' . $BOOK_LLN . '</td>
        </tr>';
    }
    ...
    $SITE_NAME2 = 'Payable par Emna Voyages';
    $SITE_NAME3 = '<table cellspacing="0" cellpadding="0" style="width: 100%; text-align: center; font-size: 12px; margin-top: 10px;">
                        <tr>
                            <td style="width:100%; padding: 8px 15px; background:none !important; color:#000; font-size: 11pt;">' . $SITE_NAME2 . '</td>
                        </tr>
                    </table>';
    $HBBOOK_REF = '';
    $HBBOOK_REF .= ' <tr>
    ...
    foreach ($BOOK_ROOMS as $BOOK_ROOM) {
        ...
        $BOOK_RR .= '
                        <table cellspacing="0" style="width: 100%; text-align: center; font-size: 12px; ">
                            ...
                            <tr>
                                <td style="width:30%;">' . $BOOK_ROOM_TYPE . '</td>
    ...
    $BOOK_RR = "";
    if ($BOOK_ST == 'hotels' || $BOOK_ST == 'hotelsbeds' || $BOOK_ST == 'carthagehotels') {
        $BOOK_ROOMS = $site->SelectFromTableWC("bookingrooms", "pidbooking", $BookingId, "pid", "ASC");
        foreach ($BOOK_ROOMS as $BOOK_ROOM) {
            $BOOK_ROOM_PID = $BOOK_ROOM['pid'];
            $BOOK_ROOM_NB = $BOOK_ROOM['numberofroom'];
            ...
            $BOOK_RR .= '
                            <table cellspacing="0" style="width: 100%; text-align: center; font-size: 11px; ">
                                <tr>
                                    <th style="width:30%;background-color: #cd015a;">Type de Chambre</th>
                                    <th style="width:30%;background-color: #cd015a;">Nom des Clients</th>
                                    <th style="width:15%;background-color: #cd015a;">Adultes</th>
                                    <th style="width:15%;background-color: #cd015a;">Enfants</th>
                                    <th style="width:10%;background-color: #cd015a;">Bébés</th>
                                </tr>
                                <tr>
        ...
        $BOOK_DET = '<table cellspacing="0" cellpadding="0" style="width: 100%; text-align: left; font-size: 12px; margin-top: 20px;">
                        ...
                        <table cellspacing="0" cellpadding="0" style="width: 100%; text-align: left; font-size: 12px; margin-top: 20px;">
                            ...
                            <tr>
                                ...
                                <td style="width:70%; text-align:left; padding-left:5px;">' . datI($BOOK_CID) . '</td>
                            </tr>
                            
                            <tr>
                                <td style="width:30%; text-align:right; padding-right:5px;">' . $activityType . ' valable jusqu\'au</td>
                                <td style="width:70%; text-align:left; padding-left:5px;">' . datI($BOOK_COD) . '</td>
                            </tr>
    ...
    if ($BOOK_ST == 'hotelbedsactivity') {
        ...
        $BOOK_CHK2 = '';
        if ($activityCommentText != '') {
            $BOOK_CHK = '<table cellspacing="0" cellpadding="0" style="width: 100%; text-align: left; font-size: 12px; margin-top: 20px;">
                            <tr>
                                <th style="width:100%; padding: 8px 15px;">OBSERVATIONS</th>
                            </tr>
                        </table>
                        <table cellspacing="0" cellpadding="0" style="width: 100%; text-align: left; font-size: 10pt; font-style:italic; margin-top: 10px;">
                            <tr>
                                <td style="width:100%; padding: 8px 15px; background:none !important; color:#000;">' . nl2br($activityCommentText) . '</td>
                            </tr>
                        </table>';
        } else {
            $BOOK_CHK = '';
    ...
    if ($BOOK_ST == 'carthagehotels') {
        ...
        $BOOK_MSGCM = stripslashes(utf8_decode($BOOKING->contactmessage));
        $BOOK_MSGCM = str_replace(', ', '<br>', $BOOK_MSGCM);
        $BOOK_CHK = '<table cellspacing="0" cellpadding="0" style="width: 100%; text-align: left; font-size: 12px; margin-top: 20px;">
                            <tr>
                                <th style="width:100%; padding: 8px 15px;background-color: #cd015a;">OBSERVATIONS</th>
                            </tr>
                        </table>
                        <table cellspacing="0" cellpadding="0" style="width: 100%; text-align: left; font-size: 10pt; font-style:italic; margin-top: 10px;">
                            <tr>
                                <td style="width:100%; padding: 8px 15px; background:none !important; color:#000;">' . nl2br($BOOK_MSGCM) . '</td>
                            </tr>
                        </table>';
    ...
    try {
        ob_start();
        include dirname(__FILE__) . '/voucher-content.php';
        $content = ob_get_clean();
        $html2pdf = new \Spipu\Html2Pdf\Html2Pdf('P', 'A4', 'en');;
        $html2pdf->pdf->SetDisplayMode('fullpage');
        $html2pdf->writeHTML($CONTENT);
        $html2pdf->Output('' . $USER . '-hotel-voucher-' . $BOOK_REF . '.pdf');
        $html2pdf->Output('vouchers/' . $USER . '-hotel-voucher-' . $BOOK_REF . '.pdf', 'F');
