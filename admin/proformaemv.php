...
if ($NB == 0) {
    ...
    if ($BOOK_ST == 'tours') {
        $BOOK_TR = 1;
        $BOOK_OPT = stripslashes(utf8_decode($BOOKING->options));
    }
    ...
    $BOOKPRICEDET = '';
    if($BOOK_PRIX_CURR != 'TND'){
	
        $BOOKPRICEDET .= '
        <table cellspacing="0" style="width: 100%; text-align: center; font-size: 11pt; ">
            <tr>
                <th style="width:33%; background-color: #cd015a; border-color: #cd015a;">Montant Achat</th>
                <th style="width:33%; background-color: #cd015a; border-color: #cd015a;">Montant Convertion</th>
                <th style="width:34%; background-color: #cd015a; border-color: #cd015a;">Montant Vente</th>
            </tr>
            <tr>
                <td>'.$BOOK_PRIX.' '.$BOOK_PRIX_CURR.'</td>
                <td>'.$BOOK_PRIX_CONVERT.'</td>
                <td>'.$BOOK_PRIX_NET.' '.$AGN_CODE.'</td>
            </tr>
        </table>';
    }else{
        $AGN_CODE= stripslashes(utf8_decode($BOOKING->agencydevise));
        if($AGN_CODE==$BOOK_PRIX_CURR){
            $BOOK_PRIX_GAIN = $BOOK_PRIX_NET - $BOOK_PRIX;
            $DeviseGain=$BOOK_PRIX_CURR;
        }else{
            $table = "devises";
            $ord   = "pid";
            $dir   = "DESC";
            $champ="CurrShortName";
         $deviseAg = $site->SelectObjectTableWC($table,$champ,$AGN_CODE,$ord,$dir);
         $CurrToTND=$deviseAg->CurrToTND;
         $prixNetToTND=$BOOK_PRIX_NET*$CurrToTND;
         $BOOK_PRIX_GAIN = $prixNetToTND - $BOOK_PRIX;
         $DeviseGain=$BOOK_PRIX_CURR;
        }
        $BOOK_PRIX_GAIN = number_format($BOOK_PRIX_GAIN, 3, '.', ' ');
        $BOOKPRICEDET .= '
        <table cellspacing="0" style="width: 100%; text-align: center; font-size: 11pt; ">
            <tr>
                <th style="width:33%; background-color: #cd015a; border-color: #cd015a;">Montant Achat</th>
                <th style="width:33%; background-color: #cd015a; border-color: #cd015a;">Montant Bénéfice</th>
                <th style="width:34%; background-color: #cd015a; border-color: #cd015a;">Montant Vente</th>
            </tr>
            <tr>
                <td>'.$BOOK_PRIX.' '.$BOOK_PRIX_CURR.'</td>
                <td>'.$BOOK_PRIX_GAIN.' '.$DeviseGain.'</td>
                <td>'.$BOOK_PRIX_NET.' '.$AGN_CODE.'</td>
            </tr>
        </table>';
    }
    ...
    try {
        ob_start();
        include dirname(__FILE__) . '/proformaemv-content.php';
